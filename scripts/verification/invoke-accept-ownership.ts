...
const result =
  invocationResult?.data?.result ?? null;
...
const createdAcceptance =
  result?.ownershipAcceptanceId
    ? await base44.entities.OwnershipAcceptance.get(
        result.ownershipAcceptanceId
      )
    : null;
...
console.log(JSON.stringify({
  test:
    "E-022 - Accountable owner accepts ownership while operation remains HOLD",
  authenticatedUser: {
    id: authUser.id,
  ...
  createdAcceptance: createdAcceptance
    ? {
        id: createdAcceptance.id,
        requirementId:
          createdAcceptance.requirementId,
        acceptedBy:
          createdAcceptance.acceptedBy,
        acceptedAt:
          createdAcceptance.acceptedAt,
        status:
          createdAcceptance.status
      }
    : null,
  createdEvent: createdEvent
    ? {
        ...
        previousState: createdEvent.previousState,
        newState: createdEvent.newState,
